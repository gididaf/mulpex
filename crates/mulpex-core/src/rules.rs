...
mod tests {
    ...
    /// The arming command in HUB_RULES is a contract with `hook.rs`: the nudge
    /// stops only when `armed/<id>` exists, and only this text creates it. If the
    /// two ever diverge, every instance re-arms forever and nothing reports it.
    #[test]
    fn hub_rules_carry_the_exact_arming_touch() {
        assert!(
            HUB_RULES.contains(r#"touch "$ARMED/$MULPEX_INSTANCE_ID""#),
            "the Monitor command that writes the armed flag must survive verbatim"
        );
