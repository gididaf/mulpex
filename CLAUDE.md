...
| [docs/explainer.md](docs/explainer.md) | Hebrew turn-summary panel: Stop→`explainreq` handshake, the transcript-flush race, the headless Sonnet child (why not `--bare`), event-not-snapshot, hard `dir="rtl"` | `explainer.rs`, `hook.rs::stop`, `ExplainerPanel.svelte`, turn extraction |
